package com.psphost.testintegrationjava;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        PSPHOSTPaymentInfo paymentInfo = getPaymentInfoOnlyRequiredParams(); // getPaymentInfoAllParams
...
//                ,theme
    ...
    // Payment Info
    PSPHOSTPaymentInfo getPaymentInfoOnlyRequiredParams() {
        return new PSPHOSTPaymentInfo(
    ...
    PSPHOSTPaymentInfo getPaymentInfoAllParams() {
        return new PSPHOSTPaymentInfo(
                PROJECT_ID, // project ID that is assigned to you
                "internal_payment_id_1", // payment ID to identify payment in your system
                1999, // 19.99
    ...
    // Additional
    void setDMSPayment(PSPHOSTPaymentInfo paymentInfo) {
        paymentInfo.setAction(PSPHOSTPaymentInfo.ActionType.Auth);
    }

    void setActionTokenize(PSPHOSTPaymentInfo paymentInfo) {
        paymentInfo.setAction(PSPHOSTPaymentInfo.ActionType.Tokenize);
    }

    void setActionVerify(PSPHOSTPaymentInfo paymentInfo) {
        paymentInfo.setAction(PSPHOSTPaymentInfo.ActionType.Verify);
    }

    void setToken(PSPHOSTPaymentInfo paymentInfo) {
        paymentInfo.setToken("token");
    }

    void setReceiptData(PSPHOSTPaymentInfo paymentInfo) {
        final String RECEIPT_DATA = "receipt data";
        paymentInfo.setReceiptData(RECEIPT_DATA);
    }

    void setRecurrent(PSPHOSTPaymentInfo paymentInfo) {
...
//                new PSPHOSTRecurrentInfoSchedule("20-10-2020",1000),
//                new PSPHOSTRecurrentInfoSchedule("20-10-2020",1000)
//        });
    ...
    void setKnownAdditionalFields(PSPHOSTPaymentInfo paymentInfo) {
        paymentInfo.setPSPHOSTAdditionalFields(new PSPHOSTAdditionalField[]{
                new PSPHOSTAdditionalField(PSPHOSTAdditionalFieldEnums.AdditionalFieldType.customer_first_name, "Mark"),
                new PSPHOSTAdditionalField(PSPHOSTAdditionalFieldEnums.AdditionalFieldType.billing_country, "US"),
        });
    }

    // if you want to hide the saved cards, pass the value - true
    void setHideSavedWallets(PSPHOSTPaymentInfo paymentInfo) {
        paymentInfo.setHideSavedWallets(false);
    }

    // For forced opening of the payment method, pass its code. Example: qiwi, card ...
    void setForcePaymentMethod(PSPHOSTPaymentInfo paymentInfo) {
        paymentInfo.setForcePaymentMethod("card");
    }

    // Setup 3D Secure 2.0 parameters
    void setThreeDSecureParams(PSPHOSTPaymentInfo paymentInfo) {
        PSPHOSTThreeDSecureInfo threeDSecureInfo = new PSPHOSTThreeDSecureInfo();

        PSPHOSTThreeDSecurePaymentInfo threeDSecurePaymentInfo = new PSPHOSTThreeDSecurePaymentInfo();
        ...
        PSPHOSTThreeDSecureGiftCardInfo threeDSecureGiftCardInfo = new PSPHOSTThreeDSecureGiftCardInfo();
        ...
        PSPHOSTThreeDSecureCustomerInfo threeDSecureCustomerInfo = new PSPHOSTThreeDSecureCustomerInfo();
        ...
        PSPHOSTThreeDSecureAccountInfo threeDSecureAccountInfo = new PSPHOSTThreeDSecureAccountInfo();
        ...
        PSPHOSTThreeDSecureShippingInfo threeDSecureShippingInfo = new PSPHOSTThreeDSecureShippingInfo();
        ...
        PSPHOSTThreeDSecureMpiResultInfo threeDSecureMpiResultInfo = new PSPHOSTThreeDSecureMpiResultInfo();
        ...
        paymentInfo.setPSPHOSTThreeDSecureInfo(threeDSecureInfo);
    }
}
