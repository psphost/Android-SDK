...
Listed below are the instructions on how to import the psphostSDK.aar library into your application

1. Download the psphostSDK.aar file;
2. Add the psphost.aar file into your project;
...
```
//psphost SDK
implementation files ('../libs/psphostSDK.aar')
//Retrofit
...
1. Create an instance of the `psphostPaymentInfo` class with all the required parameters and any number of optional parameters, for example:
```
...
- `psphostScreenDisplayMode` — object to manage display of the final page of the payment form and hide the final page if necessary.
...
```
startActivityForResult(psphostSDK.buildIntent(this,
                    paymentInfo),
                    PAY_ACTIVITY_REQUEST);
```
****
## Response processing
To receive and process response with the payment processing results you need to override the `onActivityResult` method in activity from which you started `psphostSDK`.
```
@Override
 protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
     ...
     if (requestCode == PAY_ACTIVITY_REQUEST) {
         switch (resultCode) {
             case psphostSDK.RESULT_SUCCESS:
                 // Add your code to execute when checkout completes successfully
             case psphostSDK.RESULT_CANCELLED:
                 // Add your code to execute when customer cancels checkout
             case psphostSDK.RESULT_DECLINE:
                 // Add your code to execute when checkout is declined
             case psphostSDK.RESULT_FAILED:
                 // Add your code to execute when checkout fails
                 break;
         }
         if (data != null && data.hasExtra(psphostSDK.DATA_INTENT_EXTRA_ERROR)) {
            String error = data.getStringExtra(psphostSDK.DATA_INTENT_EXTRA_ERROR);
         }
         if(data != null && data.hasExtra(psphostSDK.DATA_INTENT_EXTRA_TOKEN)) {
             String token = data.getStringExtra(psphostSDK.DATA_INTENT_EXTRA_TOKEN);
         }
         if(data != null && data.hasExtra(psphostSDK.DATA_INTENT_SESSION_INTERRUPTED)) {
             String token = data.getStringExtra(psphostSDK.DATA_INTENT_SESSION_INTERRUPTED);
         }
     }
 }
```
...
1. Download the psphostSDK.aar file;
2. Add the psphostSDK.aar file into your project;
3. Open your application module (build.gradle);
4. In the `android {}` section, add the following compilation parameters:
