package com.psphost.test_integration_kotlin

import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import android.content.Intent
import com.psphost.sdk.threeDSecure.*
import com.psphost.sdk.*
...
//              ,theme
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if(requestCode == PAY_ACTIVITY_REQUEST) {
            when (resultCode) {
                PSPHOSTActivity.RESULT_SUCCESS -> {}
                PSPHOSTActivity.RESULT_CANCELLED -> {}
                PSPHOSTActivity.RESULT_DECLINE -> {}
                PSPHOSTActivity.RESULT_FAILED -> {}
            }
    ...
    // Payment Info
    internal fun getPaymentInfoOnlyRequiredParams(): PSPHOSTPaymentInfo {
        return PSPHOSTPaymentInfo(
    ...
    internal fun getPaymentInfoAllParams(): PSPHOSTPaymentInfo {
        return PSPHOSTPaymentInfo(
            PROJECT_ID, // project ID that is assigned to you
            "internal_payment_id_1", // payment ID to identify payment in your system
            1999, // 19.99
    ...
    // Additional
    internal fun setDMSPayment(paymentInfo: PSPHOSTPaymentInfo) {
        paymentInfo.setAction(PSPHOSTPaymentInfo.ActionType.Auth)
    }

    internal fun setActionTokenize(paymentInfo: PSPHOSTPaymentInfo) {
        paymentInfo.setAction(PSPHOSTPaymentInfo.ActionType.Tokenize)
    }

    internal fun setActionVerify(paymentInfo: PSPHOSTPaymentInfo) {
        paymentInfo.setAction(PSPHOSTPaymentInfo.ActionType.Verify)
    }

    internal fun setToken(paymentInfo: PSPHOSTPaymentInfo) {
        paymentInfo.token = "token"
    }

    internal fun setReceiptData(paymentInfo: PSPHOSTPaymentInfo) {
        paymentInfo.receiptData = "receipt data"
    }

    // if you want to hide the saved cards, pass the value - true
    internal fun setHideSavedWallets(paymentInfo: PSPHOSTPaymentInfo) {
        paymentInfo.hideSavedWallets = false
    }

    // For forced opening of the payment method, pass its code. Example: qiwi, card ...
    internal fun setForcePaymentMethod(paymentInfo: PSPHOSTPaymentInfo) {
        paymentInfo.forcePaymentMethod = "card"
    }

    internal fun setRecurrent(paymentInfo: PSPHOSTPaymentInfo) {
...
//        )
    ...
    internal fun setKnownAdditionalFields(paymentInfo: PSPHOSTPaymentInfo) {
        paymentInfo.psphostAdditionalFields = arrayOf(
            PSPHOSTAdditionalField(PSPHOSTAdditionalFieldEnums.AdditionalFieldType.customer_first_name, "Mark"),
            PSPHOSTAdditionalField(PSPHOSTAdditionalFieldEnums.AdditionalFieldType.billing_country, "US")
        )
    }

    // Setup 3D Secure parameters
    internal fun setupThreeDSecureParams(paymentInfo: PSPHOSTPaymentInfo) {
        val threeDSecureInfo = PSPHOSTThreeDSecureInfo()

        val threeDSecurePaymentInfo = PSPHOSTThreeDSecurePaymentInfo()
        ...
        val threeDSecureGiftCardInfo = PSPHOSTThreeDSecureGiftCardInfo()
        ...
        val threeDSecureCustomerInfo = PSPHOSTThreeDSecureCustomerInfo()
        ...
        val threeDSecureAccountInfo = PSPHOSTThreeDSecureAccountInfo() // object with account information on record with merchant
        ...
        val threeDSecureShippingInfo = PSPHOSTThreeDSecureShippingInfo() // object that contains shipment details
        ...
        val threeDSecureMpiResultInfo = PSPHOSTThreeDSecureMpiResultInfo() // object that contains information about previous customer authentication
        ...
        paymentInfo.psphostThreeDSecureInfo = threeDSecureInfo
    }
}
